...
class UserReviewViewController: UIViewController {
    ...
    @IBAction func saveButtonPressed(_ sender: Any) {
        let data: [String: Any] = ["itemTitle" : item!.itemTitle, "itemDescription" : item!.itemDescription, "expectations" : expectationsNum, "quality" : qualityNum, "chefRating" : chefRatingNum, "recommend" : recommend, "thoughts" : thoughtsText.text ?? "", "date" :  df.string(from: date), "liked" : [], "chefEmail" : item!.chefEmail, "chefImageId" : item!.chefImageId, "itemType" : item!.typeOfService, "userEmail" : Auth.auth().currentUser!.email!, "userImageId" : Auth.auth().currentUser!.uid]
